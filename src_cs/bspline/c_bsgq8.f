...
C
	...
	implicit complex(a-h, o-z) 
     ...
     1 N, NBITS, NIB, NLMN, NLMX
      INTEGER I1MACH
      complex A, AA, AE, ANIB, ANS, AREA, B, BC, C, CE, EE, EF, EPS, ERR,
     1 EST,GL,GLR,GR,HH,SQ2,TOL,VL,VR,WORK,W1, W2, W3, W4, XT, X1,
     2 X2, X3, X4, X, H
      complex R1MACH, BVALU, G8, FUN
      DIMENSION XT(1), BC(1)
...
C***FIRST EXECUTABLE STATEMENT  BSGQ8
      IF (ICALL.cne.0) CALL XERROR( 'BSGQ8- BSGQ8 CALLED RECURSIVELY.  RE
...
C
  120 ANS = VR
      IF (((MXL.ceq.0)) .OR. (ABS(CE).LE.2.0E0*TOL*AREA)) GO TO 140
      IERR = 2
