      ...
      SUBROUTINE B2INK_mod(X,NXDATA,Y,NYDATA,FCN,LDF,KX,KY,TX,TY,BCOEF,
     & WORK,IFLAG)
...
C
      INTEGER
     *        NXDATA, NYDATA, NX, NY, LDF, KX, KY, IFLAG
      REAL
     *     X(NXDATA), Y(NYDATA), FCN(LDF,NYDATA), TX(1), TY(1), 
     *     BCOEF(NXDATA+2,NYDATA+2), WORK(100000)
...
C***FIRST EXECUTABLE STATEMENT
      ...
      IF (NXDATA .LT. 3)  GO TO 930
      IF (NYDATA .LT. 3)  GO TO 970
      IF ((KX .LT. 2) .OR. (KX .GE. NXDATA))  GO TO 940
      IF ((KY .LT. 2) .OR. (KY .GE. NYDATA))  GO TO 980
      DO 10 I=2,NXDATA
...
C  ------------
C
c      IF (IFLAG .NE. 0)  GO TO 100
c         CALL BKNOT(X,NX,KX,TX)
c         CALL BKNOT(Y,NY,KY,TY)
c 100     CONTINUE
C     
C  -------------------------------
...
C     
      ...
      IFLAG = 1
      IW = (NXDATA+2)*(NYDATA+2) + 1

c     CALL BTPCF(X,NX,FCN,LDF,NY,TX,KX,WORK,WORK(IW))
c     CALL BTPCF(Y,NY,WORK,NY,NX,TY,KY,BCOEF,WORK(IW))

      CALL BTPCF_mod(X,NXDATA,FCN, LDF,   NYDATA,TX,WORK ,WORK(IW),gk)
      CALL BTPCF_mod(Y,NYDATA,WORK,NYDATA,NXDATA,TY,BCOEF,WORK(IW),gk)

      print *,'Done from Fortran. Bye.'
      GO TO 9999
C
C  -----
C  EXITS
