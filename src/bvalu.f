...
C
  ...
  140 CONTINUE
     ...
     1 T(K)',     57, 2, 1)
      ...
      FUNCTION BVALU_C(T,A,N,K,IDERIV,X,INBV,WORK)
...
C
     ...
     1 IP1, IP1MJ, J, JJ, J1, J2, K, KMIDER, KMJ, KM1, KPK, MFLAG, N
      complex*16 FKMJ, X
      complex*16 T(N+K), WORK(3*K),A(N)
      complex*16 bvalu_c
!      DIMENSION T(1), A(N), WORK(1)
C***FIRST EXECUTABLE STATEMENT  BVALU
      BVALU_C = cmplx(0.0E0,0.0E0)
      IF(K.LT.1) GO TO 102
      IF(N.LT.K) GO TO 101
...
C     (OR, .LE. T(I+1) IF T(I) .LT. T(I+1) = T(N+1)).
      KM1 = K - 1
      CALL INTRV_C(T, N+1, X, INBV, I, MFLAG)
   ...
   10 IF (I.EQ.K) GO TO 140
      I = I - 1
      IF (real(X).EQ.real(T(I))) GO TO 10
