      COMPLEX*16 FUNCTION CB2VAL(XVAL,YVAL,IDX,IDY,TX,TY,NX,NY,
     *  KX,KY,BCOEF,WORK)
...
C
     ...
     *        IDX, IDY, NX, NY, KX, KY,j
      REAL*8
     *     XVAL, YVAL, TX(NX+KX), TY(NY+KX)
      COMPLEX*16 BCOEF(NX,NY)
...
c$$$         end do
      ...
      DO 50 K=1,KY
         KCOL = KCOL + 1
         WORK(K) = CBVALU(TX,BCOEF(1,KCOL),NX,KX,IDX,XVAL,INBVX,
     *        WORK(IW))
 50   CONTINUE
      INBV = 1
      KCOL = LEFTY - KY + 1
      CB2VAL = CBVALU(TY(KCOL),WORK,KY,KY,IDY,YVAL,INBV,WORK(IW))
