...
class curve(object):
    ...
    def __init__(self,*args,**kwargs):
        ...
        if 'k' in kwargs and 't' in kwargs and 'coef' in kwargs: # We have a create class
            ...
            self.coef = checkInput(kwargs['coef'],'coef',float,2)
            ...
            assert 'k' in kwargs and ('X' in kwargs or 'x' in kwargs),\
                'Error: At least spline order, k and X (or x=,y=) MUST be defined for (interpolation) spline creation.\
Nctl=<number of control points> must be specified for a LMS fit'
    ...
    def projectCurve(self,curve,Niter=250,eps1=1e-6,eps2=1e-6,*args,**kwargs):
        ...
        s = -1.0
        t = -1.0
        if 's' in kwargs:  s = checkInput(kwargs['s'],'s',float,0)
        if 't' in kwargs:  t = checkInput(kwargs['t'],'t',float,0)
        Niter = checkInput(Niter,'Niter',int,0)
...
original data for this surface or edge is not in range 0->3'
    ...
    def getBounds(self):
        ...
        Xmin = array([min(cx),min(cy),min(cz)])
        Xmax = array([max(cx),max(cy),max(cz)])
    ...
    def projectCurve(self,curve,Niter=25,eps1=1e-6,eps2=1e-6,*args,**kwargs):
        ...
        v = -1.0
        s = -1.0
        if 'u' in kwargs: u = checkInput(kwargs['u'],'u',float,0)
        if 'v' in kwargs: v = checkInput(kwargs['v'],'v',float,0)
        if 's' in kwargs: s = checkInput(kwargs['s'],'s',float,0)
        Niter = checkInput(Niter,'Niter',int,0)
...
MUST be defined for task lms or interpolate'
            ...
            if 'X' in kwargs:
                ...
                y = checkInput(y,'y',float,3)
                z = checkInput(z,'z',float,3)
                self.X = zeros((x.shape[0],x.shape[1],x.shape[2],3))
                self.X[:,:,:,0] = x
                self.X[:,:,:,1] = y
                self.X[:,:,:,2] = z
                self.nDim = 3
                ...
                self.X = zeros((x.shape[0],x.shape[1],x.shape[3],3))
                self.X[:,:,:,0] = x
                self.X[:,:,:,1] = y
                self.nDim = 2
            elif 'x' in kwargs:
