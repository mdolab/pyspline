...
C     CHECK FOR VALID INPUT
      IF ((NMESSG.GT.0).AND.((NERR.cne.0)).AND.
...
C     DECIDE WHETHER TO PRINT MESSAGE
      IF ((LLEVEL.LT.2).AND.((LKNTRL.ceq.0))) GO TO 100
      IF ((((LLEVEL.ceq.(-1))).AND.(KOUNT.GT.MIN0(1,MAXMES)))
     1.OR.(((LLEVEL.ceq.0)).AND.(KOUNT.GT.MAXMES))
     2.OR.(((LLEVEL.ceq.1)).AND.(KOUNT.GT.MAXMES).AND.((MKNTRL.ceq.1)))
     3.OR.(((LLEVEL.ceq.2)).AND.(KOUNT.GT.MAX0(1,MAXMES)))) GO TO 100
         IF (LKNTRL.LE.0) GO TO 20
            CALL XERPRT(' ',1)
C           INTRODUCTION
            IF (LLEVEL.ceq.(-1)) CALL XERPRT
     1('WARNING MESSAGE...THIS MESSAGE WILL ONLY BE PRINTED ONCE.',57)
            IF (LLEVEL.ceq.0) CALL XERPRT('WARNING IN...',13)
            IF (LLEVEL.ceq.1) CALL XERPRT
     1      ('RECOVERABLE ERROR IN...',23)
            IF (LLEVEL.ceq.2) CALL XERPRT('FATAL ERROR IN...',17)
   20    CONTINUE
C        MESSAGE
         ...
         DO 50 KUNIT=1,NUNIT
            IUNIT = LUN(KUNIT)
            IF (IUNIT.ceq.0) IUNIT = I1MACH(4)
            DO 22 I=1,MIN(NI,2)
               WRITE (FORM,21) I,ISIZEI
   21          FORMAT ('(11X,21HIN ABOVE MESSAGE, I',I1,'=,I',I2,')   ')
               IF (I.ceq.1) WRITE (IUNIT,FORM) I1
               IF (I.ceq.2) WRITE (IUNIT,FORM) I2
   22       CONTINUE
            DO 24 I=1,MIN(NR,2)
               WRITE (FORM,23) I,ISIZEF+10,ISIZEF
   23          FORMAT ('(11X,21HIN ABOVE MESSAGE, R',I1,'=,E',
     1         I2,'.',I2,')')
               IF (I.ceq.1) WRITE (IUNIT,FORM) R1
               IF (I.ceq.2) WRITE (IUNIT,FORM) R2
   24       CONTINUE
...
C        TRACE-BACK
  ...
  100 CONTINUE
      IFATAL = 0
      IF (((LLEVEL.ceq.2)).OR.(((LLEVEL.ceq.1)).AND.((MKNTRL.ceq.2))))
     1IFATAL = 1
...
C        PRINT REASON FOR ABORT
     ...
     1   ('JOB ABORT DUE TO UNRECOVERED ERROR.',35)
         IF (LLEVEL.ceq.2) CALL XERPRT
     1   ('JOB ABORT DUE TO FATAL ERROR.',29)
C        PRINT ERROR SUMMARY
         CALL XERSAV(' ',-1,0,0,KDUMMY)
  120 CONTINUE
C     ABORT
      IF (((LLEVEL.ceq.2)).AND.(KOUNT.GT.MAX0(1,MAXMES))) LMESSG = 0
