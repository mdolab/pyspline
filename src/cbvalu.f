      COMPLEX*16 FUNCTION CBVALU(T,A,N,K,IDERIV,X,INBV,WORK)
...
C
     ...
     1 IP1, IP1MJ, J, JJ, J1, J2, K, KMIDER, KMJ, KM1, KPK, MFLAG, N
      REAL*8 FKMJ, X
      REAL*8 T(N+K)
      COMPLEX*16 WORK(3*K),A(N),CBVALU
...
C
  101 CONTINUE
      CALL XERROR( ' CBVALU,  N DOES NOT SATISFY N.GE.K',34,2,1)
      RETURN
  102 CONTINUE
      CALL XERROR( ' CBVALU,  K DOES NOT SATISFY K.GE.1',34,2,1)
      RETURN
  110 CONTINUE
      CALL XERROR( ' CBVALU,  IDERIV DOES NOT SATISFY 0.LE.IDERIV.LT.K',
     1 49, 2, 1)
      RETURN
  120 CONTINUE
      CALL XERROR( ' CBVALU,  X IS N0T GREATER THAN OR EQUAL TO T(K)',
     1 47, 2, 1)
      RETURN
  130 CONTINUE
      CALL XERROR( ' CBVALU,  X IS NOT LESS THAN OR EQUAL TO T(N+1)',
     1 46, 2, 1)
      RETURN
  140 CONTINUE
      CALL XERROR( ' CBVALU,  A LEFT LIMITING VALUE CANN0T BE OBTAINED AT
     1 T(K)',     57, 2, 1)
      RETURN
      END
