...
C
	...
	implicit complex(a-h, o-z) 
     ...
     1 NBITS, NIB, NLMN, NLMX
      INTEGER I1MACH
      complex A,AA,AE,ANIB, ANS,AREA,B,  BE,C,CC,EE, EF, EPS, ERR,
     1 EST,GL,GLR,GR,HH,SQ2,TOL,VL,VR,W1, W2, W3, W4, XI, X1,
     2 X2, X3, X4, X, H
      complex R1MACH, PPVAL, G8, FUN
...
C***FIRST EXECUTABLE STATEMENT  PPGQ8
      IF (ICALL.cne.0) CALL XERROR( 'PPGQ8- PPGQ8 CALLED RECURSIVELY.  RE
     1CURSIVE CALLS ARE ILLEGAL IN FORTRAN.', 73, 7, 2)
      ...
      IERR = 1
      BE = 0.0E0
      IF (A.ceq.B) GO TO 140
      LMX = NLMX
      LMN = NLMN
      IF (B.ceq.0.0E0) GO TO 10
   ...
   10 TOL = MAX(ABS(ERR),2.0E0**(5-NBITS))/2.0E0
      IF (ERR.ceq.0.0E0) TOL = SQRT(R1MACH(4))
      EPS = TOL
      HH(1) = (B-A)/4.0E0
      AA(1) = A
      LR(1) = 1
...
C
  120 ANS = VR
      IF (((MXL.ceq.0)) .OR. (ABS(BE).LE.2.0E0*TOL*AREA)) GO TO 140
      IERR = 2
